...
public interface DoctorRepository extends PagingAndSortingRepository<DoctorDetails, Long> {
    ...
    @Query(value = "select new com.dashboard.doctor_dashboard.entities.dtos.DoctorListDto(dd.id,ld.name,ld.emailId,ld.profilePic,dd.speciality,dd.exp,dd.degree) from DoctorDetails dd inner join LoginDetails ld on dd.deleted = false and ld.deleted = false and dd.loginId=ld.id and dd.id!=:id")
    List<DoctorListDto> getAllDoctors(Long id);


    @Query(value = "select new com.dashboard.doctor_dashboard.entities.dtos.DoctorDropdownDto(dd.id,ld.name,ld.emailId,dd.speciality) from DoctorDetails dd inner join LoginDetails ld on dd.deleted = false and ld.deleted = false and  dd.loginId=ld.id")
    List<DoctorDropdownDto> getDoctorDetails();

    @Query(value = "select new com.dashboard.doctor_dashboard.entities.dtos.DoctorListDto(dd.id,ld.name,ld.emailId,ld.profilePic,dd.speciality,dd.exp,dd.degree) from DoctorDetails dd inner join LoginDetails ld on dd.deleted = false and ld.deleted = false and  dd.loginId=ld.id and speciality=:speciality")
    ...
    @Query(value = "select new com.dashboard.doctor_dashboard.entities.dtos.DoctorBasicDetailsDto(ld.name,ld.emailId,dd.speciality,dd.phoneNo,dd.gender,dd.age,dd.degree,dd.exp) from DoctorDetails dd inner join LoginDetails ld on dd.deleted = false and dd.id=ld.id and dd.id=:id")
